...
for s = 1 : nSubjs.ref
    ...
    % unpack the data
    for c = 1 : numCVB
        weightedCoord.ref{c}(:,s) = weightedCoord_singleSub{c};
    end
...
for i = 2 : 2: numCVB
    ...
    plot(allTps, weightedCoord.ref{i}, 'linewidth', p.LW)
...
for i = 2 : 2: numCVB
    ...
    plot(allTps, movingmean(weightedCoord.ref{i}, 5), 'linewidth', p.LW)
...
for i = 2 : 2: numCVB
    ...
    se = std(weightedCoord.ref{i},0,2) * tinv(.975, nSubjs.ref - 1);
    plot(allTps, mean(weightedCoord.ref{i},2), 'linewidth', p.LW)
    %     errorbar(allTps, mean(weightedCoord.ref{i},2), se, 'linewidth', p.LW)
